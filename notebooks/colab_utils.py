...
<style>
  ...
  .safet-btn:hover:not(:disabled) { background: #32383f; }
  .safet-btn:disabled { opacity: 0.7; cursor: default; }
  .safet-status { margin-top: 8px; font-size: 13px;
...
def _do_publish(
    ...
    """Git add/commit/push given a resolved token. Returns True if a commit was pushed."""
    ...
    # Heal notebooks written with the get_ipynb wrapper structure.
...
def publish_artifacts(
    ...
    If ``GITHUB_TOKEN_SAFET`` is set in Colab secrets, publishes immediately.
    Otherwise shows a 'Sign in & Publish' button and returns — the publish
    runs automatically once the user authenticates (non-blocking).

    Returns True if pushed synchronously, None if auth is pending.
    """
    try:
    ...
    # Fast path: secret available — publish synchronously.
    try:
        from google.colab import userdata
        token = userdata.get("GITHUB_TOKEN_SAFET")
    except Exception:
    ...
    # No secret: show button, return immediately, publish fires on click.
    ...
    output.register_callback("_safet_publish_cb", _on_token)

    display(HTML(f"""
    {_BTN_STYLE}
    <button class="safet-btn" id="safet-pub-btn">
      {_GITHUB_SVG}
      Sign in &amp; Publish
    </button>
    <div class="safet-status" id="safet-pub-status"></div>
    <script type="module">
      import {{ connectGitHub }} from 'https://neevs.io/auth/lib.js';
      const btn = document.getElementById('safet-pub-btn');
      const status = document.getElementById('safet-pub-status');
      btn.addEventListener('click', async () => {{
        ...
        try {{
          ...
          status.textContent = '';
          google.colab.kernel.invokeFunction('_safet_publish_cb', [token], {{}});
        }} catch (e) {{
          btn.disabled = false;
    ...
    """))
    print("Click 'Sign in & Publish' above to authenticate and push.")
    return None
